...
int sapool_add(nf_server_t * sev, int sock, struct sockaddr_in *addr)
{
	sapool_t *pool = (sapool_t *) sev->pool;
	int idx = -1;
...
int sapool_del(nf_server_t *sev, int idx, int alive, bool remove)
{
	sapool_t * pool = (sapool_t *) sev->pool;
	if (alive == 0) 
    {
...
void * sapool_workers(void * param)
{
	nf_server_pdata_t *pdata = (nf_server_pdata_t *) param;
	nf_server_t * sev = (nf_server_t *) pdata->server;
...
int sapool_consume(sapool_t * pool, nf_server_pdata_t * pdata)
{
	int idx;
	nf_server_t * sev = (nf_server_t *)pdata->server;
...
int sapool_pause(nf_server_t *sev)
{
